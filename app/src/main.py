...
async def model_upload(file: UploadFile):
    ...
    if key:
        ...
        return Response(
            content=json.dumps(response), media_type="application/json", status_code=201
        )
    else:
        response = {"message": "Problema no servidor"}
        return Response(
            content=json.dumps(response), media_type="application/json", status_code=500
        )
...
async def model_predict(params: ModelParams):
    ...
    if model:
        ...
        response = {
            ...
            "data": {"model_answer": int(model_answer[0])},
        }
        return Response(
            content=json.dumps(response), media_type="application/json", status_code=200
        )

    else:
        response = {"message": "Modelo especificado não existe"}
        return Response(
            content=json.dumps(response), media_type="application/json", status_code=400
        )
